...
- Class: `ChronoEngine(initial_cash)` with `run(prices, signals)` → returns equity curve (`numpy` array). Shapes required: both 2D, same shape `(n_days, n_stocks)`.
- Logic:
  ...
  - Account equity computed as cash + sum(position shares × price) per day.
...
## Integration Status
- No pipeline from plan JSON → factor evaluation → signals → `ChronoEngine` yet.
